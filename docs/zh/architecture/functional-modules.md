...
| public portal | 支持注册、登录、工作区查看与自助 API key 签发 | `services/portal-api-service`、`crates/sdkwork-api-interface-portal` |
| console | 浏览器与桌面 UI 外壳 | `console/` |
...
| 能力 | 职责 | 关键 crate |
|---|---|---|
| 网关编排 | 将 API 路由映射到 provider 执行与本地回退 | `sdkwork-api-app-gateway` |
...
| 运行时与扩展加载 | 扩展发现、ABI 边界、runtime supervision、embedded host | `sdkwork-api-app-runtime`、`sdkwork-api-extension-host`、`sdkwork-api-runtime-host`、`sdkwork-api-extension-*` |
...
| `sdkwork-api-contract-openai` | OpenAI 兼容请求与响应契约 |
...
| `scripts/dev/start-console.*` | 仅启动 UI |
| `docs/` | 文档站与历史方案记录 |
| `console/src-tauri/` | 桌面打包与宿主集成 |
...
- 上游执行和协议适配应放在 provider 或 extension runtime crate
