...
仓库内部仍然会生成一些中间产物，例如 Rust 二进制、前端 `dist/` 目录和 Tauri bundle 目录，但这些都只是构建输入，不再作为正式发布产品对外暴露。
...
desktop installer manifest 同时也是自动化和审计要消费的 machine-readable installer contract，会显式记录：

- `platform`、`arch` 和 `target`
- `artifactKind`、`installerFile` 和 `checksumFile`
- `sourceBundlePath`，也就是原始 Tauri bundle 树中实际产出的安装包路径
- 上面列出的 `embeddedRuntime.*` 内嵌运行时路径
...
- 可变运行时状态落在操作系统标准的每用户 `config`、`data` 和 `log` 目录
- 桌面壳会把访问模式持久化到 `desktop-runtime.json`
...
钩子命令负责调用平台原生签名或 notarization 工具链。仓库契约只保证安装包发现、钩子执行和证据落盘。
...
server archive manifest 也是受治理 bundle 消费方应遵循的 machine-readable install contract，会显式记录：

- `platform`、`arch`、`target`、`releaseVersion`、`archiveFile` 和 `checksumFile`
- 指向 bundle 内 `release-manifest.json` 的 `embeddedManifestFile`
- bundle 根目录 `install.sh` 与 `install.ps1` 入口的 `installers`
- `services`、`sites`、`bootstrapDataRoots` 和 `deploymentAssetRoots`，便于运维和打包自动化在不完整解包整个 bundle 树的前提下验证载荷结构
...
- 顶层 `version` 和 `type`，其中当前 catalog type 为 `sdkwork-release-catalog`
- release tag
- catalog 快照的 `generatedAt` 时间戳
- `productCount` 和 `variantCount`
- 正式 product id
...
对于正式发布包，解压受治理的 server archive 后，应优先使用 bundle 根目录安装器入口：
...
portable 安装和自定义 `<product-root>` 安装也使用同一组 bundle-root 入口：
...
源码检出场景下，仓库里的托管 `bin/install.*` 包装脚本仍然保留，作为 repository-driven 安装生成入口：
...
当需要把仓库侧的安装生成直接落到指定的 `<product-root>` 时，请使用 `--home` 或 `-Home`。生成完成后，请按照[生产部署](/zh/getting-started/production-deployment)中的方式，从已安装的 product root 执行校验和运维操作。对于正式发布包，请优先使用前面的 bundle-root `install.sh` 和 `install.ps1`。
