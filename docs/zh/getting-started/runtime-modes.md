...
SDKWork API Router 支持多种实用运行形态。关键区别不仅在于是 server 还是 desktop，还在于你是要源码级直接控制，还是使用托管脚本生命周期。
...
- 服务以独立二进制运行
- gateway、admin、portal API 直接通过 HTTP 暴露
- 如果不覆盖，二进制仍保留内建默认值
- 适合需要对单个进程做最细粒度控制的场景
...
- admin 和 portal 运行在独立的 Vite dev server
- 最适合前端联调和热更新
...
- 启停由 PID 驱动
- 默认模式是 preview，因此一启动就能使用统一浏览器入口
- 启动日志会打印统一 URL、直连服务 URL、bootstrap 提示和日志路径
...
- 你要以 portal desktop shell 作为开发态桌面入口
- 你同时还需要通过统一 Web Host 暴露浏览器访问
...
- 你在打包或部署 server 运行时
- 你需要 `systemd`、`launchd` 或 Windows Service 集成
