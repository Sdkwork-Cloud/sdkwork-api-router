...
export function listReleaseGovernanceNodeTests() {
  return listReleaseGovernanceNodeTestFiles();
}
...
export function createReleaseGovernanceNodeTestPlan({
  ...
  testFiles = listReleaseGovernanceNodeTests(),
} = {}) {
  return {
...
function buildCommandFailure(plan, result) {
  ...
  return new Error(
    `release governance node tests failed with exit code ${result?.status ?? 'unknown'} while executing ${plan.command} ${plan.args.join(' ')}${fragments.length > 0 ? `\n${fragments.join('\n')}` : ''}`,
  );
}
...
export function runReleaseGovernanceNodeTests({
  ...
  const plan = createReleaseGovernanceNodeTestPlan({
    cwd,
    env,
    nodeExecutable,
    testFiles,
