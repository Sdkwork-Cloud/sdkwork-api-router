...
export function listRustGovernanceNodeTests() {
  return listRustGovernanceNodeTestFiles();
}
...
export function createRustGovernanceNodeTestPlan({
  ...
  testFiles = listRustGovernanceNodeTests(),
} = {}) {
  return {
...
function buildCommandFailure(plan, result) {
  ...
  return new Error(
    `rust governance node tests failed with exit code ${result?.status ?? 'unknown'} while executing ${plan.command} ${plan.args.join(' ')}${fragments.length > 0 ? `\n${fragments.join('\n')}` : ''}`,
  );
}
...
export function runRustGovernanceNodeTests({
  ...
  const plan = createRustGovernanceNodeTestPlan({
    cwd,
    env,
    nodeExecutable,
    testFiles,
