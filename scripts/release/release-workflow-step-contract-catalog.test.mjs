...
test('release workflow step contract catalog governs the release baseline and preflight verification stages', async () => {
  ...
  assert.deepEqual(
    module
      .listReleaseWorkflowStepContracts()
...
test('release workflow step contract catalog publishes the exact governed late-stage workflow assertions', async () => {
  const module = await loadModule();
  ...
  assert.equal(typeof module.listReleaseWorkflowStepContracts, 'function');
  assert.deepEqual(
    module
      .listReleaseWorkflowStepContracts()
      .filter(({ id }) => expectedLateStageIds.includes(id)),
    [
      createWorkflowMatchContract(
        'governance-release-needs',
...
test('release workflow step contract catalog governs native-release dependency materialization and smoke evidence publication', async () => {
  const module = await loadModule();
  const expectedNativeRuntimeIds = [
  ...
  assert.deepEqual(
    module
      .listReleaseWorkflowStepContracts()
      .filter(({ id }) => expectedNativeRuntimeIds.includes(id)),
    [
      createWorkflowMatchContract(
        'native-release-materialize-external-deps',
...
test('release workflow step contract catalog exposes strict id-based lookup helpers for workflow consumers', async () => {
  const module = await loadModule();

  assert.equal(typeof module.findReleaseWorkflowStepContract, 'function');
  assert.equal(typeof module.listReleaseWorkflowStepContractsByIds, 'function');

  const publishSetupNodeCacheDisabled = module.findReleaseWorkflowStepContract(
    'publish-setup-node-cache-disabled',
  );
  assert.deepEqual(
    publishSetupNodeCacheDisabled,
    module
      .listReleaseWorkflowStepContracts()
      .find(({ id }) => id === 'publish-setup-node-cache-disabled'),
  );

  publishSetupNodeCacheDisabled.message = 'mutated locally';
  assert.notEqual(
    module.findReleaseWorkflowStepContract('publish-setup-node-cache-disabled').message,
    'mutated locally',
  );

  assert.deepEqual(
    module.listReleaseWorkflowStepContractsByIds([
      'rust-dependency-audit-cache-disabled',
      'governance-release-setup-pnpm',
      'governance-release-setup-node-pnpm-cache',
      'publish-setup-node-cache-disabled',
  ...
  assert.throws(
    () => module.findReleaseWorkflowStepContract('missing-release-contract'),
    /missing release workflow step contract.*missing-release-contract/i,
  );
  assert.throws(
    () => module.listReleaseWorkflowStepContractsByIds([
      'publish-setup-node-cache-disabled',
