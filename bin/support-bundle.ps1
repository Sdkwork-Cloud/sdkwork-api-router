...
if ([string]::IsNullOrWhiteSpace($SupportBundleOutput)) {
    Throw-RouterError '--output requires a value'
}
...
$arguments = @('--runtime-home', $runtimeHome, '--support-bundle-output', $SupportBundleOutput)
if ($Force) {
    $arguments += '--force'
}
if ($DryRun) {
