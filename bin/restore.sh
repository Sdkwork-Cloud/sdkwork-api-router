...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --home)
      ...
      ;;
    --source)
      [ "$#" -ge 2 ] || router_die "--source requires a value"
      SOURCE_PATH="$2"
      shift 2
      ;;
    --dry-run)
...
[ -n "$SOURCE_PATH" ] || router_die "--source requires a value"

if router_is_windows; then
  PS_SCRIPT="$(router_windows_path "$SCRIPT_DIR/restore.ps1")"
  set --
  [ -n "$RUNTIME_HOME" ] && set -- "$@" -Home "$(router_windows_cli_path "$RUNTIME_HOME")"
  [ -n "$SOURCE_PATH" ] && set -- "$@" -SourcePath "$(router_windows_cli_path "$SOURCE_PATH")"
...
set -- "$BINARY_PATH" --runtime-home "$RUNTIME_HOME" --restore-source "$SOURCE_PATH"
[ "$FORCE" = '1' ] && set -- "$@" --force
