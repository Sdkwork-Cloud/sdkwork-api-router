...
- 两个服务都提供 Prometheus 兼容的 `/metrics` 指标端点
- 两个服务都提供统一的 `x-request-id` 透传与结构化 HTTP 请求 tracing
...
两个服务都会保留调用方传入的 `x-request-id`；如果没有传入，则自动生成一个。该值会出现在响应头中，并写入独立服务输出的 HTTP 结构化请求日志。
...
两个独立服务在启动时都会初始化共享 tracing subscriber，因此每个 HTTP 请求都会以 `service`、`request_id`、`method`、`route`、`status`、`duration_ms` 这些字段输出结构化日志。
...
- gateway 与 admin 服务的 Prometheus 兼容 HTTP metrics
- gateway 与 admin 服务统一的 `x-request-id` 响应头透传与结构化 HTTP 请求 tracing
