...
test('portal marketing api client exposes coupon validation, reservation, redemption, and inventory methods', async () => {
  ...
  assert.match(types, /export interface PortalCouponValidationRequest[\s\S]*target_kind: PortalCommerceQuoteKind;/);
  assert.match(types, /export interface PortalCouponReservationRequest[\s\S]*target_kind: PortalCommerceQuoteKind;/);
