...
import { PortalTopNavigation } from '../../components/PortalTopNavigation';
import { WindowControls } from '../../components/WindowControls';

export function PortalSiteLayout({ children }: { children: ReactNode }) {
      ...
      <DesktopShellFrame
        actions={(
          <>
            ...
            <Button
              className="h-10 rounded-2xl border border-zinc-200/80 bg-white/88 px-4 text-zinc-700 shadow-[0_1px_0_rgba(15,23,42,0.04)] hover:bg-zinc-100 hover:text-zinc-950 dark:border-zinc-800 dark:bg-zinc-950/88 dark:text-zinc-200 dark:hover:bg-zinc-900 dark:hover:text-white"
              data-slot="portal-header-console-action"
              onClick={() => navigate(authenticated ? '/console/dashboard' : '/login')}
              size="sm"
              variant="ghost"
            >
              {t('Open console')}
        ...
        className="flex h-full min-h-0 flex-1 flex-col [&_[data-sdk-region='body']]:min-h-0 [&_[data-sdk-region='content']]:min-h-0 [&_[data-sdk-region='content']]:overflow-hidden [&_[data-sdk-region='header']]:border-b-0 [&_[data-sdk-pattern='desktop-title-bar']]:bg-white/72 [&_[data-sdk-pattern='desktop-title-bar']]:backdrop-blur-xl dark:[&_[data-sdk-pattern='desktop-title-bar']]:bg-zinc-950/78"
        content={(
