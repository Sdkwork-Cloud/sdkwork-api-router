...
impl Bootstrap {
    pub fn new(
        ...
        let test = options.as_ref().map(|opt| opt.test).unwrap_or_default();
        #[cfg(unix)]
        let upgrade = options.as_ref().map(|opt| opt.upgrade).unwrap_or_default();
        #[cfg(unix)]
        let upgrade_sock = _conf.upgrade_sock.clone();
