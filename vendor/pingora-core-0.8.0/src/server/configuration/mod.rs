...
//! * User and group ownership metadata for service-managed deployments
...
pub struct ServerConf {
    /// Version
    pub version: usize,
    /// Legacy daemon-mode flag retained for config compatibility.
    ///
    /// Pingora daemonization is retired in this repository. Run the process in
    /// the foreground and let a service manager own backgrounding and restarts.
    pub daemon: bool,
    /// Legacy daemon-mode stderr file path retained for config compatibility.
    pub error_log: Option<String>,
    /// Legacy daemon-mode pid file retained for config compatibility.
    pub pid_file: String,
    ...
    /// Optional service user metadata retained for config compatibility.
    pub user: Option<String>,
    /// Similar to `user`, the group metadata retained for config compatibility.
...
pub struct Opt {
    ...
    /// Legacy daemon-mode flag retained for compatibility.
    #[clap(short, long)]
    pub daemon: bool,
    ...
    pub fn validate(self) -> Result<Self> {
        if self.daemon {
            return Error::e_explain(
                ReadError,
                "Pingora daemon mode has been retired in this repository. Run the process in foreground mode and let a service manager own backgrounding and restarts.",
            );
        }
        Ok(self)
    }
...
version: 1
    ...
    #[test]
    fn test_reject_daemon_mode_from_yaml() {
        init_log();
        let conf_str = r#"
---
version: 1
daemon: true
        "#;
        let error = ServerConf::from_yaml(conf_str).expect_err("daemon mode should be rejected");
    ...
    #[test]
    fn test_reject_daemon_mode_from_cli_override() {
        init_log();
        let opt = Opt {
            daemon: true,
            ..Default::default()
