...
// NOTE: we need to keep the runtime outside async since
    ...
    pub fn new_with_opt_and_conf(raw_opt: impl Into<Option<Opt>>, mut conf: ServerConf) -> Server {
        ...
        conf = conf
            .validate()
            .unwrap_or_else(|error| panic!("invalid pingora configuration: {error}"));
    ...
    ///
    /// This function blocks forever until the server is instructed to shut down.
    ...
    /// runtimes to exit.
    ///
    /// Pingora daemon mode is not supported in this repository. Processes
    /// must run in the foreground and let a service manager own backgrounding.
    pub fn run(mut self, run_args: RunArgs) {
        ...
        if conf.daemon {
            panic!(
                "Pingora daemon mode has been retired. Run the process in foreground mode and use a service manager such as systemd, launchd, Windows Task Scheduler, Kubernetes, or the repository-managed start scripts."
            );
        }
