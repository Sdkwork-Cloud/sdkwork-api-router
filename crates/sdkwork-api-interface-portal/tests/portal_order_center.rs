...
async fn portal_paid_order_prepares_canonical_payment_artifacts() {
    ...
    assert_eq!(json[0]["order"]["order_id"], order_id);
    assert_eq!(json[0]["payment_order"]["commerce_order_id"], order_id);
    assert_eq!(
        json[0]["payment_order"]["payment_status"],
        "awaiting_customer"
    );
    assert_eq!(json[0]["payment_transactions"].as_array().unwrap().len(), 0);
    assert_eq!(json[0]["refunds"].as_array().unwrap().len(), 0);
    assert_eq!(json[0]["refundable_amount_minor"], 0);
...
async fn portal_order_center_includes_payment_and_refund_state() {
    ...
    assert_eq!(json[0]["order"]["order_id"], order_id);
    assert_eq!(
        json[0]["payment_order"]["payment_order_id"],
        payment_order.payment_order_id
    );
    assert_eq!(json[0]["payment_order"]["payment_status"], "captured");
    assert_eq!(
        json[0]["refunds"][0]["refund_order_id"],
        refund_order.refund_order_id
    );
    assert_eq!(json[0]["refunds"][0]["refund_status"], "requested");
    assert_eq!(json[0]["refundable_amount_minor"], 0);
}
...
async fn portal_order_center_includes_authorized_payment_state() {
    ...
    assert_eq!(json[0]["payment_order"]["payment_status"], "authorized");
    assert_eq!(
        json[0]["payment_order"]["fulfillment_status"],
        "authorized_pending_capture"
    );
    assert_eq!(json[0]["payment_transactions"].as_array().unwrap().len(), 1);
    assert_eq!(
        json[0]["payment_transactions"][0]["transaction_kind"],
        "authorization"
    );
    assert_eq!(json[0]["refundable_amount_minor"], 0);
}
...
async fn portal_order_center_uses_captured_amount_for_partial_capture_refunds() {
    ...
    assert_eq!(json[0]["payment_transactions"][0]["amount_minor"], 1000);
    assert_eq!(json[0]["refundable_amount_minor"], 1000);
}
...
async fn portal_order_center_lists_multiple_capture_transactions_with_aggregated_refund_capacity() {
    ...
    assert_eq!(json[0]["payment_order"]["captured_amount_minor"], 2500);
    assert_eq!(
        json[0]["payment_order"]["fulfillment_status"],
        "partial_capture_pending_review"
    );
    assert_eq!(json[0]["payment_transactions"].as_array().unwrap().len(), 2);
    assert_eq!(json[0]["refundable_amount_minor"], 2500);
}
...
async fn portal_order_center_caps_refundable_amount_after_overcapture() {
    ...
    assert_eq!(json[0]["payment_order"]["payment_status"], "captured");
    assert_eq!(json[0]["payment_order"]["captured_amount_minor"], 4000);
    assert_eq!(json[0]["payment_transactions"].as_array().unwrap().len(), 1);
    assert_eq!(json[0]["payment_transactions"][0]["amount_minor"], 4000);
    assert_eq!(json[0]["refundable_amount_minor"], 4000);
}
